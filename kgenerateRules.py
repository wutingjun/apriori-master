...
def kgenerateRules(Lk,k,supportData, minConf=0.7): #Lk:待挖掘的频繁k-项集 supportData:所有候选项集的支持度
    ...
    for freq_k_item in Lk:
        # freq_k_item是频繁k-项集中的一项,item是freqSet的每一个元素
        # 思路:对每一个频繁K(K>2)-项集中的每一项freq_k_item逐个挖掘满足支持度和置信度的关联规则.接下来就是如何从freq_k_item中找关联规则
...
def rulesFromConseq(freq_k_item, H, supportData, brl, minConf=0.7):
    ...
    m = len(H[0])
    while (len(freq_k_item) > m):   #只有len(freqSet)>m时,在calcConf计算置信度时freqSet - conseq才不会为空,说明此时可以做子项之间的置信度
